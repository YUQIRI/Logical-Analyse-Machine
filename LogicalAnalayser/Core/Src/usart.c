...
/* uart circle buffer set */

#define RX_BUFFER_SIZE 256*1024
static volatile int g_RxCplt = 0;
static volatile int g_TxCplt = 0;
static uint8_t g_rxchar;
static uint8_t g_rxBuf[RX_BUFFER_SIZE];
...
void UART1Start()
{
  /* UART_Circle_Buffer Init */
  Circle_Buf_Init(&g_uart1_rx,100,g_rxBuf);

  /* Enable recv interrupt(RXNE) */
